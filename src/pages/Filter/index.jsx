      ...
      <div className="nav-left">
        ...
        <div className="search mt-6 w-full flex">
          ...
          <button
            ...
            className="p-4 ml-2 bg-blue-900 text-white rounded hover:shadow-blue-900 shadow"
          >
            Search
          </button>
        </div>
        <h3 className="text-xl text-left font-bold text-blue-900 mt-4">
          Order
        </h3>
        <select
          onChange={handleSetOrder}
          className="outline-none max-w-xs w-full mt-4 p-4"
          name="propoty"
          id="user"
        >
          <option className="block p-4" value="name">
            Name
          </option>
          <option className="block p-4" value="age">
            Age
          </option>
        </select>
        ...
        <select
          ...
          id="user"
        >
          <option className="block p-4" value="asc">
            ASC
          </option>
          <option className="block p-4" value="desc">
            DESC
          </option>
        </select>
      </div>
      <div className="user-main">
        <h3 className="text-2xl text-left font-bold text-blue-900">
          {" "}
          List User
        </h3>
        {listUser.length > 0 ? (
          <div className="list-user mt-6">
            <table className="table-auto w-full border-collapse border border-slate-500 ">
              <thead>
                <tr>
                  <th className="border pt-4 pb-4 text-xl border-slate-600">
                    Stt
                  </th>
                  <th className="border pt-4 pb-4 text-xl border-slate-600">
                    Name
              ...
              <tbody>
                {listUser.map((item, index) => (
                  <tr key={item.id}>
                    <td className="border pt-4 pb-4 pl-2 text-xl border-slate-600">
                      {index + 1}
                    </td>
                    <td className="border pt-4 pb-4 text-xl text-center border-slate-600 ">
                      {item.name}
                    </td>
                    <td
                      className={`border pt-4 pb-4 text-xl text-center border-slate-600 ${
                        item.age >= 18 ? "text-green-500" : "text-red-500"
                      }`}
                    >
                      {item.age}
                    </td>
          ...
          <h3 className="text-xl text-left font-bold ">
            No users were found matching the key
          </h3>
        )}
      </div>
    </div>
